...
class EncoderRNN(nn.Module):    #(6884, 300, 200, 2, 0.1)
    def __init__(self, input_size, embed_size, hidden_size, n_layers=1, dropout=0.5, pre_trained_embedding=None):
        ...
        if pre_trained_embedding is not None:  # use pre-trained embedding (e.g., word2vec, glove)
            ...
            self.embedding = nn.Embedding(input_size, embed_size)   #(6884, 300)
    ...
    def forward(self, input_seqs, input_lengths, hidden=None):  #ex([[   1, 1331,  861,    2]], 4, None)
        '''
        :param input_seqs:
        ...
        packed = torch.nn.utils.rnn.pack_padded_sequence(embedded, input_lengths.to('cpu'))
...
class Attn(nn.Module):
    def __init__(self, hidden_size):    #200
        ...
        self.hidden_size = hidden_size
        self.attn = nn.Linear(self.hidden_size * 2, hidden_size)    #(400, 200)
        self.v = nn.Parameter(torch.rand(hidden_size))  #200
...
class BahdanauAttnDecoderRNN(nn.Module):    #165 200 165 2 0.1 False None
    def __init__(self, input_size, hidden_size, output_size, n_layers=1, dropout_p=0.1,
        ...
        linear_input_size = input_size + hidden_size    #165 + 200

        if self.speaker_model:
            linear_input_size += 8
        # define layers
        self.pre_linear = nn.Sequential(
            nn.Linear(linear_input_size, hidden_size),  #(365, 200)
            nn.BatchNorm1d(hidden_size),
            nn.ReLU(inplace=True)
        )
        self.attn = Attn(hidden_size)
        self.gru = nn.GRU(hidden_size, hidden_size, n_layers, dropout=dropout_p)    #200, 200, 2, 0.1

        # self.out = nn.Linear(hidden_size * 2, output_size)
        self.out = nn.Linear(hidden_size, output_size)  #200, 165
    ...
    def forward(self, motion_input, last_hidden, encoder_outputs, vid_indices=None):    #(1, 165), (2, 1, 200), (n, 1, 200), None)
        '''
        :param motion_input:
        ...
        # attention
        attn_weights = self.attn(last_hidden[-1], encoder_outputs)  # [batch x 1 x T]   배치, 1, 단어 수(n)
        context = attn_weights.bmm(encoder_outputs.transpose(0, 1))  # [batch x 1 x attn_size]
        context = context.transpose(0, 1)  # [1 x batch x attn_size] (1, batch, 200)
        ...
        rnn_input = rnn_input.unsqueeze(0) #(1, batch, 200)

        # rnn
        output, hidden = self.gru(rnn_input, last_hidden)   #(1, batch, 200) (2, batch, 200)
        # post-fc
        output = output.squeeze(0)  # [1 x batch x hidden_size] -> [batch x hidden_size]
        output = self.out(output)

        return output, hidden, attn_weights #(batch, 165) (2, batch, 200)  (batch, 1, T)

class DecoderRNN(nn.Module):    #165 200 165 2 0.1 False None
    def __init__(self, input_size, hidden_size, output_size, n_layers=1, dropout_p=0.1,
                 discrete_representation=False, speaker_model=None):
    ...
    def forward(self, motion_input, last_hidden, encoder_outputs,
                vid_indices=None):  # (1, 165), (2, 1, 200), (n, 1, 200), None)
        '''
        :param motion_input:
            motion input for current time step, in shape [batch x dim]
        :param last_hidden:
        ...
        if self.discrete_representation:
            ...
            motion_input = motion_input.view(1, motion_input.size(0), -1)  # [1 x batch x dim]
        # attention
        attn_weights = torch.ones(self.attn(last_hidden[-1], encoder_outputs).shape, requires_grad=False).to(encoder_outputs.device)  # [batch x 1 x T]   배치, 1, 단어 수(n)
        context = attn_weights.bmm(encoder_outputs.transpose(0, 1))  # [batch x 1 x attn_size]
        context = context.transpose(0, 1)  # [1 x batch x attn_size] (1, batch, 200)
        ...
        return output, hidden, attn_weights  # (batch, 165) (2, batch, 200)  (batch, 1, T)
class Generator(nn.Module): #args, 165
    def __init__(self, args, motion_dim, hidden_size=None, discrete_representation=False, speaker_model=None):
        super(Generator, self).__init__()
        self.output_size = motion_dim   # 165
        self.n_layers = args.text_n_layers   #2
        if hidden_size is None :
            hidden_size = args.text_hidden_size
        self.discrete_representation = discrete_representation  #False
        self.decoder = BahdanauAttnDecoderRNN(input_size=motion_dim,    #165
                                              hidden_size=hidden_size, #200
                                              output_size=self.output_size, #165
                                              n_layers=self.n_layers,   #2
                                              dropout_p=args.dropout_prob,  #0.1
                                              discrete_representation=discrete_representation,
                                              speaker_model=speaker_model)
        """
        self.decoder = DecoderRNN(input_size=motion_dim,    #165
                                              hidden_size=hidden_size, #200
                                              output_size=self.output_size, #165
                                              n_layers=self.n_layers,   #2
                                              dropout_p=args.dropout_prob,  #0.1
    ...
    def forward(self, z, motion_input, last_hidden, encoder_output, vid_indices=None):  #None, (1, 165), (2, 1, 200), (n, 1, 200), None
        if z is None:
            input_with_noise_vec = motion_input #(1, 165)
        else:
        ...
        return self.decoder(input_with_noise_vec, last_hidden, encoder_output, vid_indices) #(1, 165), (2, 1, 200), (n, 1, 200), None)
        #return (batch, 165) (2, batch, 200)  (batch, 1, T)


class Seq2SeqNet(nn.Module):    # args, 165, 40, 6884, 300, (6884, 300), None
    def __init__(self, args, pose_dim, n_frames, n_words, word_embed_size, word_embeddings, speaker_model=None):
        super().__init__()
        self.encoder = EncoderRNN(
            n_words, word_embed_size, args.text_hidden_size, args.text_n_layers,  #(6884, 300, 200, 2)
            dropout=args.dropout_prob, pre_trained_embedding=word_embeddings)
        self.decoder = Generator(args, pose_dim, speaker_model=speaker_model)   #args, 165,

        self.n_frames = n_frames
        self.n_pre_poses = args.n_pre_poses
        self.pose_dim = pose_dim

    def forward(self, in_text, in_lengths, poses, vid_indices): #ex([[   1, 1331,  861,    2]], 4, (1, 10, 165))
        # reshape to (seq x batch x dim)
        in_text = in_text.transpose(0, 1)   #ex(4, 1)
        poses = poses.transpose(0, 1)   #(10, 1, 165)

        outputs = torch.zeros(self.n_frames, poses.size(1), self.decoder.output_size).to(poses.device)
                    #40, 64, 165)
        # run words through encoder
        encoder_outputs, encoder_hidden = self.encoder(in_text, in_lengths, None)   #ex([[1], [1331], [861], [2]], 4, None)
        ...
        for t in range(1, self.n_frames):   #0번 빼고 39번 반복
